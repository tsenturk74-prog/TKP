...
function tkpOrderedForLeg(x){
  ...
  return dedupeEkuri(pool.slice().sort((a,b)=>
    tkpCouponCorePriorityScore(x?.r,b,pool)-tkpCouponCorePriorityScore(x?.r,a,pool) ||
    (Number(b.agf)||0)-(Number(a.agf)||0) || tkpFrozenOdsResultRank(a)-tkpFrozenOdsResultRank(b) ||
...
function tkpApplyKulisPriorityToCoupon(coupon,role){
  ...
  for(const leg of coupon.legs){
    ...
    const ranked=pool.map((h,index)=>{
      ...
      const support=ev?.available===true?Math.max(0,Math.min(1,Number(ev.ratio)||0))*(1+priority):0;
      return {h,index,support};
...
async function tkpBuildCouponSetAdaptiveCriticalAsync(raceResults,budgets={},options={}){
  ...
  try{
    ...
    if(globalThis.TKP_ARCHIVE_COUPON_PLANNER){
      const candidate=globalThis.TKP_ARCHIVE_COUPON_PLANNER.build(raceResults,Math.min(policy.main,policy.surprise),{current:true});
      if(options.currentReplayFast!==true&&globalThis.TKP_COUPON_COVERAGE){
        globalThis.TKP_COUPON_COVERAGE.planSet(candidate);
        // Güncel replay'de optimize(), olasılık modelini her ayak/rol için yeniden
        // açıp 3.000+ koşuluk arşiv taramasına girebiliyor. Kanonik planın kapsamı
        // zaten hazırdır; replay için bu ikinci ortak-kapsam aramasını çalıştırma.
        if(options.currentReplayFast!==true)globalThis.TKP_COUPON_COVERAGE.optimize(candidate);
      }
      if(globalThis.TKP_COUPON_POLICY)globalThis.TKP_COUPON_POLICY.enforce(candidate);
...
function recalcCouponCost(res){
  res.combos = res.legs.reduce((m,l)=>m*Math.max(1,l.picks.length),1);
...
function tkpSideBetStrategyPlanSignature(x){
  ...
  const dbSig=typeof tkpFastDbSignature==='function'?tkpFastDbSignature(db):`${db?.files?.length||0}:${db?.races?.length||0}`;
  return dbSig+'|'+[r.id||r.file_id||'',r.leg||'',...pool.map(h=>[h?.horse_no,h?.score,h?.prediction_score_snapshot,h?.altili_winner_score,h?.agf,h?.agf_rank,h?.ypuan,h?.value_score,h?.hndkp_rank,h?.jbyg,h?.bmb,h?.odb,h?.tr_ganyan,h?.tr_puan,h?.tr_ganyan_source,h?.tr_source,0,h?.expert_consensus_total,h?.expert_positive_votes].join(':'))].join('|');
}
...
function tkpSideBetProductPlayDecision(product,race,rankings,pools){
  ...
  try{
    ...
    if(exactGate?.exact===true){
      return {play:false,strong:false,label:String(exactGate.label||'SHADOW'),mode:String(exactGate.mode||'SHADOW'),verified:false,
        reason:`Tam bilet canlı kilidi · ${Number(exactGate.events)||0}/${Number(exactGate.minimum)||150} · ${String(exactGate.reason||'Otomatik aktivasyon kapalı.')}`};
    }
  }catch(_){ }
  const verified=tkpVerifiedSideBetRecommendation(product);
  if(verified){
...
function tkpSideBetTicketHtmlSignature(x,nextRace){
  ...
  const couponPart=['main','alt','surprise'].map(key=>{
    ...
    const row=(coupon?.legs||[]).find(item=>Number(item?.leg)===leg);
    return `${key}:${(row?.picks||[]).map(h=>String(h?.horse_no??h)).join(',')}`;
  }).join('|');
  return `${Number(_sideBetSignatureEpoch)||0}|${leg}|${Number(nextRace?.r?.id)||Number(nextRace?.r?.leg)||0}|${couponPart}`;
}
...
function sideBetTicketForRace(x, stats, nextRace=null, options={}){
  ...
  if(nextRace && Number(nextRace.r.leg)===Number(x.r.leg)+1 && doubleIsOffered){
    ...
    const doubleDecision=tkpSideBetProductPlayDecision('cifte',x.r,null,[championDoubleA,championDoubleB]);
...
async function tkpCaptureExactSideBetTickets(snapshot,raceResults){
  ...
  for(let index=0;index<rows.length;index++){
    ...
    sideBetTicketForRace(x,stats,next,{exactSpecs:specs});
...
function saveAutomaticCouponSnapshot(raceResults,coupons,budgets,source){
  ...
  if(backtestEligible){
    try{ if(typeof tkpForwardTrackingAttachCouponSnapshot==='function') tkpForwardTrackingAttachCouponSnapshot(snapshot); }catch(_e){}
    try{ if(typeof tkpWeeklyModelTrackerAttachCouponSnapshot==='function') tkpWeeklyModelTrackerAttachCouponSnapshot(snapshot); }catch(_e){}
