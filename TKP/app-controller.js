...
function tkpForwardTrackingAttachCouponSnapshot(snapshot,options={}){
  ...
  if(changed&&options.persist!==false&&globalThis.__tkpBulkPipelineActive!==true) try{saveDB(false);}catch(_e){} return changed;
}
...
function tkpForwardTrackingReasonsHTML(){
  ...
  return `<div id="tkpForwardReasonsDailyPanel" class="card tkpForwardReasonTrackingText"><h2>📝 İleri Takip · Neden Kaçtı</h2><p class="muted">Seçili günün ${rows.length} kanıtlı kaçışı gösterilir. Tüm arşivde öğrenmeye alınan kaçış: ${view.missCount}.</p><div class="tkpForwardReasonTrackingNotes">${notes}</div></div>`;
}
...
function tkpCommentatorPerformanceHTML(){
  ...
  // Eski arşivde olmayan ama canlı/pre-race kanıt toplamaya başlamış kaynakları da
  // göster. Önceki yalnız histBy anahtarları Misli.com gibi yeni yorumcuları görünmez
  // yapıyordu; veri yok sanılıyordu. Tarihsel ve canlı kanıt tek listede birleşir,
  // fakat her satır kendi örneklemini açıkça etiketler.
...
function parseResultHtmlPayloadCandidates(text, filename){
  ...
  // 1) TJK Yarış Sonuçları: resmi sonuç HTML. R16.81 izolasyonu: sonuç yükleme
  // akışı YALNIZ 2. Altılı'yı hedefler; 1. Altılı adayı üretilmez, böylece yanlış
  // altılının sonucu hedef kayda karışamaz.
  if(/gunluk-GunlukYarisSonuclari/i.test(text)){
    try{ addCandidate(parseTjkResultsHTML(text,filename,2)); }catch(_e){}
  }
  ...
  if(/gunluk-GunlukYarisProgrami-AtAdi/i.test(text)){
    try{ addCandidate(parseTjkProgramHTML(text,filename,2)); }catch(_e){}
  }
...
async function tkpReadResultPayloadFromFile(f){
  ...
  // R16.81: sonuç yükleme akışı yalnız 2. Altılı'yı hedefler. Kaynak dosya hangi
  // altılı numarasını taşırsa taşısın, hedef kayıt 2. Altılı olarak sabitlenir;
  // eşleşme katmanı yanlış altılıyı WRONG_ALTILI ile reddeder.
  p.file.altili_no=2;
...
function tkpSchedulePredictionContextUpgrade(displayRaces,logPayload,renderOptions,renderGeneration){
  ...
  const run=async()=>{
    ...
    try{
      ...
      if(typeof tkpYield==='function')await tkpYield();
      tkpPreparePredictionStats();
      if(typeof tkpYield==='function')await tkpYield();
      if(typeof hippodromeSignalStats==='function')hippodromeSignalStats();
      if(typeof globalSignalStats==='function')globalSignalStats();
      if(token!==_tkpPredictionContextUpgradeSeq||renderGeneration!==_tkpPredictionRenderGeneration)return false;
      const active=document.querySelector('#tkpTabs .tab.active')?.dataset?.pane;
      if(active==='prediction'&&globalThis.__tkpCouponBuildInProgress!==true){
        // Tam bağlam tazelemesi kupon kartları oluştuktan sonra da çalışabilir;
        // yalnız tahmin alanını tazeler, görünür üç kuponu sıfırlamaz veya
        // collector kritik fazını yeniden açmaz.
        await renderPredictionScreen(displayRaces,logPayload,{...(renderOptions||{}),deferredContextRefresh:true,criticalPhase:false,collectorRunId:'',preserveCouponDom:true});
      }
      return true;
...
async function renderPredictionScreen(displayRaces2, logPayload, renderOptions={}){
      ...
      // 1.1.213'te düzeltilmiş hız sözleşmesi: İleri Takip ve haftalık öğrenme
      // render/yükleme yolunda görünmeden çalıştırılmaz. Bu analizler kendi manuel
      // panel/düğme akışında hazırlanır; veri aktarımı ve ilk ekran boyası rehin alınmaz.
        ...
        firstLookHtml = `<div class="card firstLookOverviewCard" style="border-left:5px solid #0f4c81;">
          ...
          <div class="tableWrap firstLookOverviewTable"><table>
            ...
            <tbody>${firstLookRows.map(x=>{
              ...
              if(fastFirstPaint){
                confHtml='<span class="muted" style="font-size:10.5px;">Kupon hazırlığında hesaplanacak</span>';
              }else try{
                ...
                if(d){
                  ...
                  // İlk Bakış ana güveni her zaman ham TEK motoru yüzdesidir (rawPct).
                  // Tarihsel kalibrasyon burada ana değeri değiştirmez; yalnız aşağıdaki
                  // 'outcome' satırında ek kanıt olarak gösterilir.
                  const pct=rawPct;
...
$('#bulletinFile').onchange = async (e) => {
          ...
          try {
            ...
            const card=(coupon,title)=>{
              ...
              const rows=(coupon.legs||[]).slice().sort((a,b)=>Number(a.r?.leg)-Number(b.r?.leg)).map(l=>{
                ...
                const pickGrid=picks.length
                  ? `<span class="tkpSharePickGrid">${picks.map(no=>`<span class="tkpSharePick">${esc(no)}</span>`).join('')}</span>`
                  : '<span class="tkpShareEmpty">-</span>';
                const tek=picks.length===1?`<span class="tkpShareTek">TEK</span>`:'';
                return `<tr><td>${esc(l.r?.leg)}. AYAK</td><td>${pickGrid}${tek}</td></tr>`;
              }).join('');
  ...
  async function tkpDownloadCompressedBackup(options={}){
    ...
    try{
      ...
      if(btn)btn.textContent=`✅ .tkbz ${usedFolder?'klasöre kaydedildi':'indirildi'} · ${expectedBackupCounts.files} dosya / ${expectedBackupCounts.races} koşu / ${expectedBackupCounts.prediction_log||0} tahmin`;
    }
    ...
    return new Promise((resolve,reject)=>{
      ...
      worker.onmessage=event=>{
        ...
        if(message.type==='phase'){
          ...
          const current=`${db?.files?.length||0} dosya / ${db?.races?.length||0} koşu / ${db?.prediction_log?.length||0} tahmin`;
          const next=`${message.counts?.files||0} dosya / ${message.counts?.races||0} koşu / ${message.counts?.prediction_log||0} tahmin`;
  ...
  if($('#restoreTKPBackup'))$('#restoreTKPBackup').onchange=async(e)=>{
    ...
    if(typeof Worker!=='undefined'){
      try{
        ...
        if(restoreLabel)restoreLabel.textContent=`✅ ${result.counts.files} dosya / ${result.counts.races} koşu / ${result.counts.prediction_log||0} tahmin aktarıldı${resolvedMsg}`;
        alert(`Sıkıştırılmış yedek başarıyla geri yüklendi · ${result.counts.files} dosya / ${result.counts.races} koşu / ${result.counts.prediction_log||0} tahmin${resolvedMsg}.`);
        try{ localStorage.removeItem('tkpAutoRecalcRulesAfterRestore'); }catch(_e){}
        location.reload();
