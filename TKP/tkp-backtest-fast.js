...
(function(global){
  ...
  function algorithmDevelopmentComparisonHTML(result){
    ...
    const sideMap=[['Çifte','cifte'],['Sıralı İkili','sirali_ikili'],['Sıralı Üçlü','sirali_uclu'],['Tabela','tabela'],["Sıralı 5'li",'sirali5li']];
    const sideRows=sideMap.map(([name,key])=>{
      const b=archive?.sidebetSummary?.[name]?.finalAll||{},c=result.side?.[key]||{},bn=num(b.total),bh=num(b.hits),br=bn?bh/bn:0,cn=num(c.total),ch=num(c.ok),cr=cn?ch/cn:0;
      const broi=Number.isFinite(Number(b.roi))?Number(b.roi)*100:null,croi=Number.isFinite(Number(c.roi))?Number(c.roi):null;
      const delta=(cr-br)*100;
      return `<tr><td><b>${escLocal(name)}</b></td><td class="num">${bh}/${bn} · %${fmt2Local(br*100)}</td><td class="num">${ch}/${cn} · %${fmt2Local(cr*100)}</td><td class="num">${pp(delta)}</td><td class="num">${broi==null?'—':'%'+fmt2Local(broi)}</td><td class="num">${croi==null?'—':'%'+fmt2Local(croi)}</td></tr>`;
    }).join('');
