...
function tkpHybridAlternativePortfolio(built){
  // R17.6: Normal kuponun güncel çekirdeğini bozma. Alternatif iki kuponda
  // güncel sıra ile eski şampiyonun yarış-öncesi TKP/score omurgasını aynı
  // maliyet içinde birleştir. Sonuç/ikramiye alanları burada ASLA okunmaz.
  if(!built)return built;
  const legacyScore=h=>{
    const vals=[h?.prediction_score_snapshot,h?.pre_race_tkp_score,h?.tkp_display_score,h?.score]
      .map(Number).filter(Number.isFinite);
    return vals.length?vals[0]:0;
  };
  const swapOne=(coupon,role)=>{
    if(!coupon||coupon.error||coupon.readOnlySnapshot||!Array.isArray(coupon.legs))return;
    let changed=0;
    for(const leg of coupon.legs){
      const picks=Array.isArray(leg?.picks)?leg.picks:[];
      if(picks.length<=1||leg?.protectedFlag)continue;
      const pool=(leg?.allHorses?.length?leg.allHorses:(leg?.r?.horses||[])).filter(h=>h&&!isNonRunner(h));
      if(pool.length<3)continue;
      const legacy=pool.slice().sort((a,b)=>legacyScore(b)-legacyScore(a)||(Number(b?.agf)||0)-(Number(a?.agf)||0));
      const selected=new Set(picks.map(h=>String(h?.horse_no)));
      let candidates=role==='alt'?legacy.slice(2,6):legacy.slice(0,4);
      if(role==='alt')candidates=candidates.sort((a,b)=>{
        const as=(Number(a?.bmb)===1?3:0)+(Number(a?.priorSurpriseHits)||0)+(Number(a?.condSurpriseHits)||0);
        const bs=(Number(b?.bmb)===1?3:0)+(Number(b?.priorSurpriseHits)||0)+(Number(b?.condSurpriseHits)||0);
        return bs-as||legacyScore(b)-legacyScore(a);
      });
      const cand=candidates.find(h=>!selected.has(String(h?.horse_no)));
      if(!cand)continue;
      const displayRank=new Map((leg.displayOrder||pool).map((h,i)=>[String(h?.horse_no),i]));
      const removable=picks.slice().reverse().find(h=>{
        const no=String(h?.horse_no);
        return (displayRank.get(no)??999)>=2 && !(leg.protectedValueNos instanceof Set&&leg.protectedValueNos.has(no))
          && !(leg.protectedSignalNos instanceof Set&&leg.protectedSignalNos.has(no))
          && !(leg.protectedBombNos instanceof Set&&leg.protectedBombNos.has(no));
      });
      if(!removable)continue;
      const at=picks.indexOf(removable);
      if(at<0)continue;
      picks[at]=cand;
      leg.picks=dedupeEkuri(picks,leg.r,'surprise',true);
      changed++;
      if(changed>=1)break;
    }
    if(changed){
      recalcCouponCost(coupon);
      coupon.hybridPortfolio={version:'R17.6',role,legacyPreRaceLane:true,changes:changed,resultLeakage:false};
    }
  };
  swapOne(built.alt,'alt');
  swapOne(built.surprise,'expert');
  return built;
}

function tkpEnforceFinalSingleContract(built){
  // Son optimizerlerden SONRA çalışan sert görünür kupon sözleşmesi.
  // Her aktif Altılı: minimum 1, maksimum 2 TEK. Teksiz kupon yayınlanmaz.
  const policy={minSingles:1,maxSingles:2};
  for(const [key,role] of [['main','normal'],['alt','alt'],['surprise','expert']]){
    const c=built?.[key];
    if(!c||c.error||c.disabled||c.readOnlySnapshot||!Array.isArray(c.legs))continue;
    tkpEnsureCouponShape(c,role);
    let singles=c.legs.filter(l=>(l?.picks||[]).length===1);
    if(singles.length<policy.minSingles){
      c.error='Geçerli TEK bulunamadı; teksiz Altılı kupon yayınlanmadı.';
      c.singleContract={...policy,actual:singles.length,pass:false,version:'R17.6'};
      continue;
    }
    if(singles.length>policy.maxSingles){
      tkpEnsureCouponShape(c,role);
      singles=c.legs.filter(l=>(l?.picks||[]).length===1);
    }
    for(const leg of singles){
      leg.protectedFlag=true;leg.minCoverage=1;leg.maxCoverage=1;
      leg.confidencePlan={...(leg.confidencePlan||{}),finalSingleLock:true};
    }
    recalcCouponCost(c);
    if(Number(c.cost)<TKP_COUPON_FEE_POLICY.minBudgetTL-1e-9){
      tkpRaiseCouponCostToMinimum(c,TKP_COUPON_FEE_POLICY.minBudgetTL,TKP_COUPON_FEE_POLICY.maxBudgetTL);
      tkpEnsureCouponShape(c,role);
      singles=c.legs.filter(l=>(l?.picks||[]).length===1);
      recalcCouponCost(c);
    }
    const pass=singles.length>=policy.minSingles&&singles.length<=policy.maxSingles;
    c.singleContract={...policy,actual:singles.length,pass,version:'R17.6'};
    if(!pass&&!c.error)c.error='TEK sözleşmesi sağlanamadı; kupon yayınlanmadı.';
  }
  return built;
}
...
function tkpFinalizeCouponSetFast(built,key,safeBudgets,started){
  ...
  tkpHybridAlternativePortfolio(built);
  tkpEnforceFinalSingleContract(built);
  if(typeof tkpCouponCheck==='function')tkpCouponCheck();
...
function tkpSideBetProductPlayDecision(product,race,rankings,pools){
  ...
  // R17.2 gerçek maliyet/geri dönüş kapısı: geçmişte yüksek hit veya tek büyük
  // ödeme görülse bile gerçek ticket ROI'si belirgin negatif olan ürün otomatik
  // OYNA olamaz. Yeni saat-kilitli forward gate üstte her zaman önceliklidir.
  try{
    const roiGate=globalThis.TKP_V296_COST_ROI_POLICY?.recommendation?.(product);
    if(roiGate?.block===true){
      return {play:false,strong:false,label:String(roiGate.label||'PAS'),mode:String(roiGate.mode||'PAS'),verified:true,
        roiGate:true,reason:String(roiGate.reason||'R17.2 gerçek ROI kapısı')};
    }
  }catch(_){ }
  const verified=tkpVerifiedSideBetRecommendation(product);
  if(verified){
